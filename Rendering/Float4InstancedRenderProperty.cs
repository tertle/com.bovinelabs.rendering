// <copyright file="Float4InstancedRenderProperty.cs" company="BovineLabs">
...
namespace BovineLabs.Systems.Rendering
{
        ...
        where T : struct, IInstancedRenderProperty<float4>
    {
        ...
        public Float4InstancedRenderProperty()
        {
            ...
            this.Data = NativeArrayUnsafeUtility.ConvertExistingDataToNativeArray<float4>(ptr, this.managedData.Length, Allocator.Invalid);
        ...
        {
            this.handle.Free();
            this.managedData = Array.Empty<Vector4>();
            base.Dispose();
        }
        ...
        public override void AddData(in ArchetypeChunk chunk, in NativeArray<VisibleIndex> visible, int instanceStart, int copyCount)
        {
            ...
            var src = new NativeSlice<T>(chunkData).SliceConvert<float4>();
            var dest = new NativeSlice<float4>(this.Data, instanceStart, copyCount);
        ...
        public override void ApplyAndFree(MaterialPropertyBlock block, int instanceCount)
        {
            ...
            try
            {
                ...
                block.SetVectorArray(NameId, this.managedData);
            }
            finally
            {
        ...
        public override void ApplyPerInstance(MaterialPropertyBlock block, int index)
        {
            if (this.lastWrite < index)
                block.SetVector(NameId, this.DefaultValue);
            else
                block.SetVector(NameId, this.managedData[index]);
        }
        ...
        public override bool FetchDefault(Material material)
        {
            ...
            this.DefaultValue = material.GetVector(NameId);
            return true;
        }
    }
}
